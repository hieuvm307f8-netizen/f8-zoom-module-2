...
export async function homeScript() {
  ...
  if (access_token) {
    try {
      ...
      if (authRessponse.data) {
        greetingEl.innerHTML = `Xin chào, ${authRessponse.data.name} <i class="fa-solid fa-heart" style="color: red;"></i>`;
      }
    } catch (error) {
      console.warn("Không thể lấy thông tin người dùng (có thể token hết hạn).");
      greetingEl.innerHTML = `Chào mừng bạn đến với thế giới âm nhạc!`;
  ...
  try {
    ...
    moodListEl.innerHTML = moodListResponse.data.items
      .map((item) => `<a href="/moods/${item.slug}" data-navigo class="btn">${item.name}</a>`)
      .join("");
  } catch (err) { console.error("Lỗi lấy danh sách Mood:", err); }
  ...
  try {
    ...
    quickPickEl.innerHTML = quickPickResponse.data
      .map((item) => `
        ...
        `).join("");
  } catch (err) { console.error("Lỗi lấy Quick Pick:", err); }
  ...
  try {
    ...
    SuggestAlbumEl.innerHTML = suggestAlbRequest.data
      .map((item) => `
        ...
        `).join("");
  } catch (err) { console.error("Lỗi lấy Album gợi ý:", err); }
  ...
  try {
    ...
    todayHitEl.innerHTML = todayHitResponse.data
      .map((item) => `
        ...
        `).join("");
  } catch (err) { console.error("Lỗi lấy Top Hits:", err); }
  ...
  try {
    ...
    countryList.innerHTML = countryResponse.data
      .map((item) => `
        ...
        `).join("");
  } catch (err) { console.error("Lỗi lấy Nhạc Việt:", err); }
}